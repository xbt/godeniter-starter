...
func NewArticleService() *ArticleService {
	...
	s.Create("欢迎使用 Godeniter 2.0 框架", "基于纯 Go 标准库与依赖注入容器打造的零依赖 Web 框架，兼具高性能与 CodeIgniter 优雅易用性。", "admin@godeniter.dev", "/uploads/images/sample_cover.svg")
	...
	s.Create("企业级数据安全：敏感信息脱敏与 XSS 过滤", "集成手机号、邮箱、身份证自动脱敏工具，配合 HTML 转义防御跨站脚本攻击，护航业务系统稳定上线。", "security@godeniter.dev", "/uploads/images/sample_cover.svg")
...
func (s *ArticleService) Create(title, content, author, coverURL string) *models.Article {
	...
	// 业务安全：使用框架内置 str.XSSFilter 防御跨站脚本攻击
	safeContent := str.XSSFilter(content)
...
// Update 更新文章 (支持更新封面与 XSS 安全过滤)
func (s *ArticleService) Update(id int, title, content, author, coverURL string) (*models.Article, bool) {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	if content != "" {
		a.Content = str.XSSFilter(content)
	}
	if author != "" {
		a.Author = author
	}
